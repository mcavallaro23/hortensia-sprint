...
window.navigate = function(screen) {
  ...
  switch (screen) {
    ...
    case 'athletes':
      ...
      break;
    case 'photocells':
      screenDiv.innerHTML = "<h2>Scan Photocells</h2><button onclick='scanPhotocells()'>Scan & Connect</button>";
      break;
    default:
...
function startMultiAthleteTest(testIndex) {
  ...
  finishedAthletes = [];
  alert("Ready to start test for: " + selectedAthletes.join(", "));
  // Aquí iría la carga del test real
}
...
function loadTest(testIndex) {
  ...
  screen.innerHTML = `
    ...
    <button onclick="loadTest(${testIndex})">Reset Athlete</button>
...
function recordImpulse() {
  ...
  if (impulsesReceived.length >= expectedCaptures + 1) {
    ...
    if (testQueue.length > 0) {
      setTimeout(() => loadTest(tests.findIndex(t => t.name === tests[0].name)), 2000);
    }
  }
}

function updateAthleteLists() {
  document.getElementById('waitingList').innerHTML = testQueue.map(a => `<li>${a}</li>`).join('');
  document.getElementById('finishedList').innerHTML = finishedAthletes.map(a => `<li>${a.name}: ${a.time}</li>`).join('');
}
